...
from decidra.utils.global_vars import get_config
...
class NotificationService:
    ...
    def __init__(self):
        ...
        """
        # 各渠道的 Webhook URL (从 Notification 配置节读取)
        self._wechat_url = get_config('Notification', 'WechatWebhookUrl', '')
        self._feishu_url = get_config('Notification', 'FeishuWebhookUrl', '')

        # Telegram 配置
        self._telegram_config = {
            'bot_token': get_config('Notification', 'TelegramBotToken', ''),
            'chat_id': get_config('Notification', 'TelegramChatId', ''),
        }

        # 邮件配置 (从 Email 配置节读取)
        email_sender = get_config('Email', 'Sender', '')
        email_password = get_config('Email', 'Password', '')
        subscription_list = get_config('Email', 'SubscriptionList', [])
        if isinstance(subscription_list, str):
            import json
            try:
                subscription_list = json.loads(subscription_list)
            except (json.JSONDecodeError, TypeError):
                subscription_list = []
        self._email_config = {
            'sender': email_sender,
            'password': email_password,
            'receivers': subscription_list or ([email_sender] if email_sender else []),
        }

        # 自定义 Webhook 配置
        custom_urls_str = get_config('Notification', 'CustomWebhookUrls', '')
        if custom_urls_str:
            self._custom_webhook_urls = [u.strip() for u in custom_urls_str.split(',') if u.strip()]
        else:
            self._custom_webhook_urls = []

        # 消息长度限制（字节）
        self._feishu_max_bytes = int(get_config('Notification', 'FeishuMaxBytes', 20000))
        self._wechat_max_bytes = int(get_config('Notification', 'WechatMaxBytes', 4000))
