...
@test.command('test-lowfreq-data')
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
@click.option('--enable', is_flag=True, help='启用测试（需要FutuOpenD环境）')
def test_lowfreq_data(verbose: bool, enable: bool):
    """运行富途API低频数据测试"""
    if enable:
        os.environ['FUTU_TEST_ENABLED'] = 'true'
        print_warning("已启用低频数据测试，请确保FutuOpenD正在运行")
    
    print_info("开始运行富途API低频数据测试...")
    success = run_tests('tests.test_futu_lowfreq_data', verbosity=2 if verbose else 1)
    
    if success:
        print_success("低频数据测试完成！")
    else:
        print_error("低频数据测试失败！")
    
    sys.exit(0 if success else 1)


@test.command('test-subscription')
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
@click.option('--enable', is_flag=True, help='启用测试（需要FutuOpenD环境）')
def test_subscription(verbose: bool, enable: bool):
    """运行富途API订阅测试"""
    if enable:
        os.environ['FUTU_TEST_ENABLED'] = 'true'
        print_warning("已启用订阅测试，请确保FutuOpenD正在运行")
    
    print_info("开始运行富途API订阅测试...")
    success = run_tests('tests.test_futu_subscription', verbosity=2 if verbose else 1)
    
    if success:
        print_success("订阅测试完成！")
    else:
        print_error("订阅测试失败！")
...
@test.command('test-quote-integration')
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
@click.option('--enable', is_flag=True, help='启用测试（需要FutuOpenD环境）')
def test_quote_integration(verbose: bool, enable: bool):
    """运行富途API行情集成测试"""
    if enable:
        os.environ['FUTU_TEST_ENABLED'] = 'true'
        print_warning("已启用行情集成测试，请确保FutuOpenD正在运行")
    
    print_info("开始运行富途API行情集成测试...")
    success = run_tests('tests.test_futu_quote_integration', verbosity=2 if verbose else 1)
    
    if success:
        print_success("行情集成测试完成！")
    else:
        print_error("行情集成测试失败！")
    
    sys.exit(0 if success else 1)


@test.command('test-quote-manager')
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
def test_quote_manager(verbose: bool):
    """运行富途API行情管理器测试"""
    print_info("开始运行富途API行情管理器测试...")
    success = run_tests('tests.test_futu_quote_manager', verbosity=2 if verbose else 1)
    
    if success:
        print_success("行情管理器测试通过！")
        sys.exit(0)
    else:
        print_error("行情管理器测试失败！")
        sys.exit(1)


@test.command('test-strategy')
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
def test_strategy(verbose: bool):
    """运行策略测试"""
    print_info("开始运行策略测试...")
    success = run_tests('tests.test_strategy', verbosity=2 if verbose else 1)
...
@test.command('test-data-engine')
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
def test_data_engine(verbose: bool):
    """运行数据引擎测试"""
    print_info("开始运行数据引擎测试...")
    success = run_tests('tests.test_data_engine', verbosity=2 if verbose else 1)
    
    if success:
        print_success("数据引擎测试通过！")
        sys.exit(0)
    else:
        print_error("数据引擎测试失败！")
        sys.exit(1)
...
@click.option('--verbose', '-v', is_flag=True, help='详细输出')
@click.option('--enable-integration', is_flag=True, help='启用集成测试')
def test_all(verbose: bool, enable_integration: bool):
    """运行所有测试"""
    print_info("开始运行所有测试...")
    
    # 测试模块列表
    test_modules = [
        ('tests.test_config_manager', '配置管理器测试'),
        ('tests.test_futu_api', '富途API单元测试'),
        ('tests.test_futu_quote_manager', '行情管理器测试'),
        ('tests.test_strategy', '策略测试'),
        ('tests.test_data_engine', '数据引擎测试'),
    ]
    
    # 需要FutuOpenD环境的集成测试
    integration_modules = [
        ('tests.test_futu_integration', '富途API集成测试'),
        ('tests.test_futu_lowfreq_data', '低频数据测试'),
        ('tests.test_futu_subscription', '订阅测试'),
        ('tests.test_futu_quote_integration', '行情集成测试'),
    ]
    ...
    for module, name in test_modules:
        print_info(f"运行 {name}...")
        success = run_tests(module, verbosity=2 if verbose else 1)
        results[name] = success
    
    # 运行集成测试
    if enable_integration:
        os.environ['FUTU_TEST_ENABLED'] = 'true'
        print_warning("已启用集成测试")
...
def start_monitor(stocks: Optional[str], refresh: int, mode: str):
    ...
    try:
        ...
        click.echo("  A: 添加股票")
        click.echo("  D: 删除股票")
        click.echo("  Enter: 进入分析界面")
        click.echo("  Tab: 切换标签页")
        click.echo("  H: 显示帮助")
...
def monitor_status():
    ...
    try:
        ...
        try:
            import monitor_app
            print_success("监控应用模块: 可用")
        except ImportError as e:
            print_error(f"监控应用模块: 不可用 ({e})")
        
        # 检查Textual框架
        try:
            import textual
            print_success("Textual框架: 已安装")
        except ImportError:
            print_error("Textual框架: 未安装")
            click.echo("  安装命令: pip install textual")
        
        # 检查相关模块
        modules_to_check = [
            ('base.monitor', '数据模型'),
            ('monitor.data_flow', '数据流管理'),
            ('monitor.futu_interface', '富途接口'),
            ('monitor.indicators', '技术指标'),
        ...
        for module_name, description in modules_to_check:
            try:
                ...
                print_error(f"{description}: 不可用 ({e})")
        
        # 检查配置
        if get_config_manager is not None:
            try:
                config_manager = get_config_manager()
                config = config_manager._config_data
                stocks_config = config.get('monitored_stocks', {})
                ...
                click.echo(f"\n配置状态:")
                print_success("配置管理器: 可用")
                if monitored_stocks:
                    print_success(f"监控股票: {len(monitored_stocks)}只")
                    for stock in monitored_stocks[:5]:  # 只显示前5只
                        click.echo(f"  • {stock}")
                    if len(monitored_stocks) > 5:
                        click.echo(f"  • ... 还有{len(monitored_stocks)-5}只")
                else:
                    print_warning("监控股票: 列表为空")
                    
            except Exception as e:
                print_warning(f"配置管理器: 初始化失败 ({e})")
        else:
            print_error("配置管理器: 不可用")
        
        # 检查富途API连接
        click.echo(f"\n富途API状态:")
        try:
            defaults = get_futu_defaults_from_config()
            print_info(f"配置地址: {defaults['host']}:{defaults['port']}")
            
            # 尝试连接测试
            if create_client:
                try:
                    client = create_client(host=defaults['host'], port=defaults['port'])
                    with client:
                        print_success("富途API: 连接正常")
                except Exception as e:
                    print_warning(f"富途API: 连接失败 ({e})")
            else:
                print_warning("富途API: 模块不可用")
                
        except Exception as e:
            print_error(f"富途API: 检查失败 ({e})")
        
        click.echo(f"\n运行建议:")
        click.echo("  1. 确保FutuOpenD程序正在运行")
        click.echo("  2. 使用 'python cli.py monitor config --add HK.00700' 添加监控股票")
        click.echo("  3. 使用 'python cli.py monitor start' 启动监控界面")
